...
async function cliOptions() {
  ...
  const rawImagesOption =
    options.images && typeof options.images === "string"
      ? options.images
      : undefined;
  let date =
    options.date && typeof options.date === "string" ? options.date : undefined;
  let images = rawImagesOption
    ? rawImagesOption.split(",").map((v) => v.trim())
    : [];
  if (images.length === 0) {
