...
async function resolveImage(
  props: Speaker & { clearCache?: boolean },
...
export async function getEventWithSpeakers(
  eventFileName: string,
  directoryPath: string = "src/content",
) {
  if (!eventFileName.endsWith(".md"))
    eventFileName = `${eventFileName}-event.md`;
  const eventData = await getEventData(
    relativeDirectory(`${directoryPath}/events/${eventFileName}`),
  );
  const presentations = await Promise.all(
    eventData.presentations.map(async (presentationFile) => {
      const presentationData = await getPresentationData(
        relativeDirectory(
          `${directoryPath}/presentations/${presentationFile}.md`,
        ),
      );
      const speakerData = await getSpeakerData(
        relativeDirectory(
          `${directoryPath}/speakers/${presentationData.speaker}.md`,
        ),
      );
  ...
  const speakers = await Promise.all(
    presentations.map(async ({ speaker }) => ({
      ...speaker,
      buffer: await resolveImage(speaker),
    })),
  );
  ...
  return {
    ...
    time:
      date.getHours() >= 12
        ...
        : `${date.getHours()}am`, // Format hours for AM/PM.
  };
}
...
export async function speakersImage(date: string): Promise<SpeakersImageData> {
  ...
  const speakers = event.speakers.map(({ buffer, name }) => ({
    image: buffer,
    name,
  }));
  return {
