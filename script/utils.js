...
const flags = () => {
  ...
  class Require {
    ...
    static get presentations() {
      ...
      if (
        !flags.presentations.every((presi) =>
          isValidPresentation(flags.date, presi),
        )
      ) {
        throw new Error(
          "Invalid presentation file format format. Date must be in yyyy-mm-dd format.",
        );
      }
  ...
  class Sanitize {
    ...
    static get presentations() {
      try {
        ...
        return flags.presentations.filter((presi) =>
          isValidPresentation(flags.date, presi),
        );
      }
    }
...
async function nextEventTitle() {
  ...
  const no = parseInt(
    frontMatter.title.replace("Astoria Tech Meetup #", "").trim(),
  );
...
export async function mkevent() {
  ...
  const merge = deepmerge(
    frontMatter,
    deepCleaner.clean({ ...input, presentations: p }),
  );
  await writeContent(fileName, merge, content);
}
